...
Record of analysis for *Sula sula*.
-------------------------------------
...
results <- spocc::occ(query = sp, from = "gbif", limit = 100, has_coords = TRUE)
...
selPoly <- mcp(occs.xy)
selPoly <- rgeos::gBuffer(selPoly, width = selPoly_buf_dd)
...
Originally, used WorldClim
(<a href="http://www.worldclim.org/" class="uri">http://www.worldclim.org/</a>)
bioclimatic dataset at resolution of 10 arcmin.

#### sdmpredictors

* [sdmpredictors quickstart guide](https://cran.r-project.org/web/packages/sdmpredictors/vignettes/quickstart.html)

```{r}
shelf(DT, here, sdmpredictors) # ?sdmpredictors
...
# TODO: check out MARSPEC layers
lyrs <- layers %>% 
  filter(
    dataset_code == "Bio-ORACLE",
    derivation == "mean")
datatable(lyrs)

# get layers
options(sdmpredictors_datadir= dir_data)
envs <- load_layers(lyrs)

# set cell resolution
envRes <- res(envs)[1] # 0.1666667

# get WorldClim bioclimatic variable rasters
# envs <- raster::getData(name = "worldclim", var = "bio", res = 10, path = dir_data)
...
* [Modeling methods — R Spatial](https://rspatial.org/raster/sdm/6_sdm_methods.html)
...
occs_bb <- st_bbox(occs_pts)

plot(envs[[1]])
plot(occs_pts["presence"], add = T)
```
...
occs <- occs[!is.na(locs.vals), ]  # removed 2
```
...
```{r}
shelf(glue, readr, stringr)

# define the vector of regularization multipliers to test
rms <- seq(1, 2, 1)

# iterate model building over all chosen parameter settings
mx_rds <- here(glue("data/{sp %>% str_replace_all(' ', '_')}.maxent.rds"))

if (!file.exists(mx_rds)){
  e <- ENMeval::ENMevaluate(
    occs.xy, envsBgMsk, bg.coords = bg.xy, RMvalues = rms, fc = c('L', 'LQ'), 
...
dismo::response(evalMods[["L_1"]], var = names(envs))
```
...
pred_v <- values(pred)
# TODO: use threshold(evaluate(p=p, a=a)) based on test data predictions
table(pred_v > 0.01)
table(pred_v > 0.1)
table(pred_v > 0.5)
```
...
TODO:
- limit background environmental sampling to similar depths or distance from shore
- speed up by caching results, a la mx_rds
- update Shiny app to use sdmpredictors